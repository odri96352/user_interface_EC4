...
from classification_model_to_add_13_new_labels import ClassificationModel
...
class SmilesClassificationModel(ClassificationModel):
    def __init__(
        ...
        **kwargs,
        ...
        print('in /BECPred/models_add_1_new_label.py')
        ...
        self.args = self._load_model_args(model_name)
        print(f"smile self arg: {self.args}")
        print(f"num_labels: {num_labels}, len(self.args.labels_list): {len(self.args.labels_list)}")
        if isinstance(args, dict):
            self.args.update_from_dict(args)
        ...
        if self.args.labels_list:
            print(f"models_add_1_new_label.py line 382, self.args.labels_list: {self.args.labels_list}")
            if num_labels:
    ...
    def add_labels_smile_classification(self, num_labels_to_add):
        print(f"function added by audrey in models_add_13_new_labels.py to add labels to a classfication model")
        for i in range(num_labels_to_add):
            # print(f"models.py line 382, self.args.labels_list before: {self.args.labels_list}")
            # print(f"self.model.args.eval_batch_size: {self.args.eval_batch_size}")
            # adding the new label to the label list:
            last_label = self.args.labels_list[-1] + 1
            self.args.labels_list.append(last_label)
            # adding the new label to the label map:
            self.args.labels_map[last_label] = last_label
            # adding 1 label to ClassificationModel for the loss calculation
            self.num_labels += 1
            # print(f"models.py line 382, self.args.labels_list after: {self.args.labels_list}")
            #Extend the weights by creating a new tensor that consists of the
            ...
            self.config.id2label[last_label] = f"LABEL_{str(last_label)}"
            self.config.label2id[f"LABEL_{str(last_label)}"] = last_label
        
        return
    
    def print_self_args(self):
        print("in /home/audrey/Documents/Audrey_INSA_internship/personal_gitub/InsaInternship/BECPred/models_add_1_new_label.py")
        print(f"smile self arg: {self.args}")
        print(f"num_labels: {self.num_labels}, len(self.args.labels_list): {len(self.args.labels_list)}")
        print(f"self.args.labels_map: {self.args.labels_map}")
        print(f"self.config smiles classification: {self.config}")
        return
