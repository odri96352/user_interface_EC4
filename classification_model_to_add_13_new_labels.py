...
class ClassificationModel:
    ...
    def train_model(
        ...
        **kwargs,
        ...
        print(f"Added an initialisation of loss in /BECPred/classification_model.py on line 540")
        ...
        **kwargs,
        ...
        for i, batch in enumerate(
            ...
            end_index = (
                ...
                if i != (n_batches - 1)
                else len(eval_dataset)+1
            )
            # print("in /home/audrey/Documents/Audrey_INSA_internship/personal_gitub/InsaInternship/BECPred/classification_model.py")
            # print(f"logits.detach().cpu().numpy(): {logits.detach().cpu().numpy()}")
            # print(f"logits.detach().cpu().numpy().shape(): {np.shape(logits.detach().cpu().numpy())}")
            # print("Audrey added a +1 in else len(eval_dataset)+1")
    ...
    def predict(self, to_predict, multi_label=False):
        ...
        """
        print("in classification_model")
        # print("self.args")
        # print(self.args)
        print("self.weight")
        print(self.weight)
        print("added self.config.output_hidden_states")
    ...
    def get_attention(self, input_ids, token_type_ids):
        print(" Audrey added a function in /home/audrey/Documents/Audrey_INSA_internship/personal_gitub/InsaInternship/BECPred/classification_model.py to get attention weights")
        return self.model(input_ids, token_type_ids=token_type_ids)
    
    def add_labels(self):
        print(" Audrey added a function in /home/audrey/Documents/Audrey_INSA_internship/personal_gitub/InsaInternship/BECPred/classification_model.py to add more labels to the model")
        print(f"model.classifier: {self.model.classifier}")
        ...
        self.model.classifier.weight = nn.Parameter(torch.cat((self.model.classifier.weight, torch.randn(1,308)),0))
        #We do the same for the bias:
        self.model.classifier.bias = nn.Parameter(torch.cat((self.model.classifier.bias, torch.randn(1)),0))
        return
    
    def change_class_weights(self,new_weights):
